...
namespace entities
{
    ...
    std::shared_ptr<Entity> createPlayerSelf(const shared::Player& pbPlayer, sf::Vector2f viewSize, std::unordered_set<std::shared_ptr<sf::Texture>>& textures)
    {
        ...
        //
        // A player ship has the following components
        entity->addComponent(std::make_unique<components::Position>(sf::Vector2f(pbPlayer.position().center().x(), pbPlayer.position().center().y()), pbPlayer.position().orientation()));
        entity->addComponent(std::make_unique<components::Size>(viewSize));
        entity->addComponent(std::make_unique<components::Sprite>(spriteShip));
