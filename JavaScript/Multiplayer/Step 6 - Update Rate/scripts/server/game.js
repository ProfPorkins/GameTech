...
const CLIENT_UPDATE_RATE_MS = 100;      // How often to update the connected clients with game state (in ms)
let quit = false;
let activeClients = {};
let newMissiles = [];
let activeMissiles = [];
    ...
    while (!processMe.empty) {
        ...
        switch (input.message.type) {
            case NetworkIds.INPUT_THRUST:
                ...
                // Need to update the lastUpdateTime because multiple thrust messages might come through
...
function updateClients(elapsedTime) {
    //
    // Demonstrating that it is possible to update the clients at a different/slower
    // rate than the server-side game simulation update rate.
    lastClientUpdate += elapsedTime;
    if (lastClientUpdate < CLIENT_UPDATE_RATE_MS) {
        return;
    }
    //
    ...
    // Add back in any leftover time from going negative.
    lastClientUpdate = lastClientUpdate - CLIENT_UPDATE_RATE_MS;
}
